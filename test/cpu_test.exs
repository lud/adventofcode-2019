defmodule CpuTest do
  ...
  test "addition, multiplication" do
    assert Cpu.State.to_list(Cpu.run!("1,0,0,0,99")) == [2, 0, 0, 0, 99]
    assert Cpu.State.to_list(Cpu.run!("2,3,0,3,99")) == [2, 3, 0, 6, 99]
    assert Cpu.State.to_list(Cpu.run!("2,4,4,5,99,0")) == [2, 4, 4, 5, 99, 9801]
    assert Cpu.State.to_list(Cpu.run!("1,1,1,4,99,5,6,0,99")) == [30, 1, 1, 4, 2, 5, 6, 0, 99]
  end
  ...
  test "immediate mode" do
    assert Cpu.State.to_list(Cpu.run!("1002,4,3,4,33")) == [1002, 4, 3, 4, 99]
  end
      ...
      {:ok, client} =
        Cpu.run(program,
          io: fn
            ...
            {:output, value, iostate} ->
              assert expected === value
              send(self(), {:ouputed, value})
          end
        )

      assert_receive({:ouputed, ^expected})
    end
  end
  ...
  test "program that copies itself" do
    ...
    Cpu.run!(program,
      io: fn {:output, value, _} ->
        send(self(), {:out, tag, value})
        :ok
      end
    )

    assert expected == capture_ouput(tag)
  end

  defp capture_ouput(tag, acc \\ []) do
    receive do
      {:out, ^tag, value} -> capture_ouput(tag, [value | acc])
    after
      0 -> :lists.reverse(acc)
    end
  end
  ...
  test "day9 healthcheck" do
    ...
    assert [3_638_931_938] = capture_ouput(tag)
    ...
    assert [86025] = capture_ouput(tag)
  end
end
